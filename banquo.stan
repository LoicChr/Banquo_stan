functions{
  matrix geninv(matrix G, int n, real epsilon){
    ...
    // use the top left of the M matrix to hold the inverse of LL' LL
    M = rep_matrix(0,n,n);
    M[1:r,1:r] = inverse(crossprod(block(LL,1,1,n,r)));
...
model{
  ...
  for( i in 1:S ){
    for( j in i:S ){
      if( i == j ){
        ...
        sampled_alpha[i,i] += alphaij_intercept;
      }else{
  ...
  banquo = traitspace * geninv(sampled_alpha, S, 10e-8)';
  ...
  // use hellinger distance to give something that scales with likelihood
  for( i in 1:Nsites ){
    // normalization
    banquo_agg[i] = banquo_agg[i]/sum(banquo_agg[i]);

    // contribution to the likelihood based on hellinger dissimilarity
